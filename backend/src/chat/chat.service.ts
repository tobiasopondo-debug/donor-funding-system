...
export class ChatService {
  ...
  private async assertCanAccessChat(userId: string, role: UserRole, orgId: string) {
    const org = await this.prisma.organization.findUnique({ where: { id: orgId } });
    if (!org || org.status !== OrganizationStatus.APPROVED) throw new NotFoundException();
  ...
  async postMessage(userId: string, role: UserRole, orgId: string, body: string) {
    await this.assertCanAccessChat(userId, role, orgId);
    const trimmed = body.trim();
    if (!trimmed.length) throw new BadRequestException('Message cannot be empty');
    const room = await this.getOrCreateRoom(orgId);
