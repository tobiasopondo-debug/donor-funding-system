...
export class FilesService {
  ...
  private bucketForKind(kind: FileKind) {
    return kind === FileKind.VERIFICATION_DOC ? this.privateBucket : this.publicBucket;
  }

  async presign(userId: string, role: UserRole, dto: import('./dto/file.dto').PresignDto) {
  ...
  async confirm(userId: string, role: UserRole, dto: import('./dto/file.dto').ConfirmFileDto) {
    if (role !== UserRole.NGO_USER) throw new ForbiddenException();
    const org = await this.prisma.organization.findUnique({ where: { ownerUserId: userId } });
    if (!org) throw new BadRequestException('Create organization first');
    const expectedBucket = this.bucketForKind(dto.kind);
    if (dto.bucket !== expectedBucket) throw new BadRequestException('Invalid bucket');
    if (dto.kind === FileKind.PROJECT_IMAGE) {
      if (!dto.projectId) throw new BadRequestException('projectId required');
      const project = await this.prisma.project.findFirst({
        where: { id: dto.projectId, orgId: org.id },
